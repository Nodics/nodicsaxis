  ...
  record3: {
    ...
    properties: {
      ...
      source: {
        ...
        hash: 'ec9cf4d637cad9c3ad16803a61831e19dcec0730882dd45a716d207ad242b3a8',
        version: '0.3.15',
      },
      previous: {
        title: 'Architecture and Repository Boundaries',
  ...
  record11: {
    ...
    properties: {
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "**Build query and preview.** Axis calls the owning module's Workbench search route to preview records. The preview is deliberately bounded and read-only. It helps the employee verify the query before generating a file, but it is not the export authority.",
        ...
        {
          kind: 'paragraph',
          text: 'To customize export safely, change the owning backend schema/search behavior, export access policy, nExport rendering/provider services, or nMedia storage configuration. Axis may improve the query builder and result presentation, but it must not query databases directly, render authoritative business files from browser-only data, or decide media storage paths.',
        },
        {
          kind: 'paragraph',
      ...
      source: {
        ...
        hash: '357c7bc6c116588cf884909497d1397fb6c947ab02601caebcf43995abf9f014',
        version: '0.3.15',
      },
      previous: {
        title: 'Module Health',
  ...
  record12: {
    ...
    properties: {
      ...
      blocks: [
        ...
        {
          kind: 'paragraph',
          text: 'The implemented route is `/media-management/*`. It provides a governed workspace shell and section cards that explain the active nMedia capability areas. Each section route is meaningful even before its full CRUD grid is implemented:',
        },
        {
          kind: 'unordered-list',
          items: [
        ...
        },
        {
          kind: 'unordered-list',
          items: [
            '**Source type**, which is the business purpose derived from backend folder metadata, such as data imports, data exports, product media, content media, business documents, or general media;',
            '**Visibility**, such as private, public, or future signed delivery;',
            '**Status**, such as ready, consumed, retired, or failed;',
        ...
        {
          kind: 'paragraph',
          text: 'Axis also paginates the visible table so a long result set does not become one unmanageable page. The backend media schema/search contract remains the authority for record retrieval. As the number of media records grows into thousands or millions, nMedia should expose server-side paging, sorting, filtering, and search-index backed queries through the same backend-owned contract. Axis should keep the same user experience and pass the selected filters to that contract; it must not create a browser-only media index or read storage folders directly.',
        },
        {
          kind: 'paragraph',
          text: 'The same section also supports governed upload. The employee selects an upload purpose, such as import sources, data export files, CMS content assets, product media assets, or utility files. Axis asks nMedia for the upload policy, lets the employee choose a local file, and posts the multipart upload to the nMedia `/storage/upload` endpoint. nMedia validates the folder, file type, size, checksum, provider, and storage key. Axis receives the returned media code and refreshes the media list. Axis does not choose the filesystem folder, does not generate the storage key, and does not persist media metadata directly.',
        },
        {
          kind: 'paragraph',
        ...
        {
          kind: 'paragraph',
          text: 'The **Media Formats** section is also operational. Axis discovers the `mediaFormat` schema from nMedia and shows reusable presentation or processing formats such as original, thumbnail, desktop, mobile, zoom, or import file. The screen presents format code, name, purpose, description, width, height, and a combined dimensions view. Formats help backend and frontend teams use consistent business vocabulary for media variants without making Axis transform images or own storefront rendering behavior.',
        },
        {
          kind: 'paragraph',
        ...
        {
          kind: 'paragraph',
          text: 'When an employee selects a media set, Axis also loads the set composition from the nMedia-owned `mediaSetEntry` schema. The detail panel shows each linked variant with its media code, optional format code, variant role, locale, dimensions, position, and lifecycle status. This keeps the business view clear: the set describes the logical group, each entry describes a specific variant, and each variant still points to an owned media record. Axis does not duplicate variant ownership or infer image behavior; it asks nMedia for the set entries using a backend filter on the selected set code.',
        },
        {
          kind: 'paragraph',
        ...
        {
          kind: 'paragraph',
          text: 'The **Storage and Delivery** section now provides a read-only policy inspection view. Axis calls the nMedia `/storage/policy` API with small safe probe descriptors for known folder purposes such as import sources, data export files, CMS assets, product assets, and utility files. The result shows folder-level upload rules: folder code, business label, visibility, allowed extensions, allowed MIME types, maximum file size, and checksum algorithm.',
        },
        {
          kind: 'paragraph',
          text: 'This screen is intentionally not a provider diagnostic console yet. The current backend contract does not expose provider internals, and Axis must not infer them. It does not call the storage-location endpoint, does not generate storage keys, and does not display absolute filesystem paths, bucket names, certificates, or credentials. It also does not decide whether a folder is local, NAS, S3, Azure, Google Cloud Storage, FTP, or a partner provider. nMedia owns that decision.',
        },
        {
          kind: 'paragraph',
        ...
        },
        {
          kind: 'ordered-list',
          items: [
            'Axis asks nMedia, "what files are allowed for this folder?"',
            'nMedia validates the folder and returns safe policy metadata.',
            'Axis displays only the safe policy metadata.',
        ...
        {
          kind: 'paragraph',
          text: 'Axis deliberately does not display backend-resolved full paths. If a file can be opened, the UI uses the nMedia delivery URL or the nMedia content endpoint, not a filesystem path. This keeps local storage, NAS, cloud storage, and future signed URL providers behind the backend media contract.',
        },
        {
          kind: 'paragraph',
          text: 'The **Media** record detail view follows the same rule. When a selected media record is a public image in a deliverable lifecycle state, Axis can render a small preview by calling the nMedia content delivery URL. For other file types, Axis offers an open or download action only when the backend record is public and in a deliverable state. Private and signed files are not opened directly from the browser until nMedia exposes the proper authorized or signed delivery contract. Axis must never convert `fullPath`, `relativePath`, bucket keys, or storage keys into browser links.',
        },
        {
          kind: 'paragraph',
        ...
        },
        {
          ...
          items: [
            ...
            'Axis renders the route only when the authenticated bootstrap contains the entry.',
            ...
            'Storage policy inspection uses nMedia `/storage/policy` and does not call storage-location or upload APIs unless that workflow is explicitly being executed.',
      ...
      source: {
        ...
        hash: 'd71a8a16fcbd819d15b79b9b91d366999e5020194df7b1c5745eb72a4bdd90f2',
        version: '0.3.15',
      },
      previous: {
        title: 'Imports and Exports Workspace',
  ...
  record14: {
    ...
    properties: {
      ...
      source: {
        ...
        hash: '4fc371c3d394f3997d7d38a43a725077343cd52caf882cf85bd4ca66c1dd2517',
        version: '0.3.15',
      },
      previous: {
        title: 'Axis Feature Delivery Checklist',
