  ...
  record12: {
    ...
    properties: {
      ...
      blocks: [
        ...
        {
          kind: 'paragraph',
          text: 'Folder policy editing is intentionally gated by the generated schema contract. If the discovered `mediaFolder` schema does not advertise `GENERATED_CRUD` update permission for the employee session, Axis shows the policy as read-only and directs administrators back to backend-approved media configuration or the Schema Workbench. When update is advertised, Axis exposes a small policy action panel for visibility, maximum upload size, and retention days. The panel submits only those fields through the generated `mediaFolder` schema mutation; it does not edit storage prefixes, resolved paths, provider secrets, provider configuration, or browser-side policy rules. nMedia remains responsible for validation, routing, provider behavior, tenant policy, and persistence.',
        },
        {
          kind: 'paragraph',
          text: "For full generated CRUD, Media Management links to Schema Workbench instead of duplicating the generic record form. The handoff URL is `/schema-workbench?module=media&schema=mediaFolder`; when the backend advertises create permission, Axis may also link to `/schema-workbench?module=media&schema=mediaFolder&mode=create`. Schema Workbench still uses the discovered nMedia schema and the employee's authorized operations, so the handoff does not create a second media-folder authority.",
        },
        {
          kind: 'paragraph',
      ...
      source: {
        ...
        hash: '260ee8c040544632fbd87edb102902bbf361cb6a977a4e26d8aa7b86ad4c2463',
        version: '0.3.15',
      },
      previous: {
        title: 'Imports and Exports Workspace',
